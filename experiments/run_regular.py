#!/usr/bin/env python3
"""
Experiment script for random regular graphs.

This script generates k-regular graphs with various parameters and runs all MVC
algorithms to collect comparative performance data.

Usage:
    python run_regular.py [--nodes N1 N2 ...] [--graphs G]
    
Example:
    python run_regular.py --nodes 100 200 300 --graphs 10
"""
...
from quantum_walk_mvc.graph_generators import generate_regular_graphs


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description='Run MVC experiments on random regular graphs'
    )
    parser.add_argument(
        '--nodes', 
        type=int, 
    ...
    parser.add_argument(
        ...
        type=int, 
        default=10,
        help='Number of unique graphs per (n, k) configuration'
    )
    parser.add_argument(
        '--t-max', 
        type=float, 
...
def main():
    """Main entry point for regular graph experiments."""
    args = parse_args()
    
    print("=" * 60)
    print("Quantum Walk MVC - Regular Graphs Experiment")
    print("=" * 60)
    ...
    if args.output is None:
        filename_suffix = f"N{args.nodes[0]}-{args.nodes[-1]}_G{args.graphs}"
        results_filename = f'results_regular_{filename_suffix}.csv'
    else:
    ...
    print("\n[1/2] Generating random regular graphs...")
